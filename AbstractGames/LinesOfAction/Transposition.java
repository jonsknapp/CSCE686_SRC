...
public class Transposition {
    ...
    long[][] hashBoard = new long[2][LOABoard.BOARD_SIZE*LOABoard.BOARD_SIZE]; //range of 0..1, and 0..63
    ...
    public Transposition() {
      ...
      // Load the Transposition table values into array here.
  	  String inFileName = "zobrist"+new Integer(LOABoard.BOARD_SIZE).toString()+".hsh";
  	  try {
    	...
    	GZIPInputStream gzis = new GZIPInputStream(fis);     // Uncompress
  		...
  		// Read in the table.
  		for ( int i = 0; i <=1; i++)
  		  for (int j = 0; j <= (LOABoard.BOARD_SIZE * LOABoard.BOARD_SIZE)-1;j++ ) {
      ...
      if ( hashBoard[0][0] == 0 ) {
    	...
    	String outFileName = "zobrist"+new Integer(LOABoard.BOARD_SIZE).toString()+".hsh";
    	try {
      ...
      //put in white
      for (int i=1; i<= LOABoard.BOARD_SIZE-2; i++) {
        currentHashKey ^= hashBoard[PLAYER_WHITE][LOABoard.BOARD_SIZE*i];
        currentHashKey ^= hashBoard[PLAYER_WHITE][LOABoard.BOARD_SIZE*i+(LOABoard.BOARD_SIZE-1)];
      }

      //put in black
      for (int i=1; i <= LOABoard.BOARD_SIZE-2; i++) {
        currentHashKey ^= hashBoard[PLAYER_BLACK][i];
        currentHashKey ^= hashBoard[PLAYER_BLACK][i+LOABoard.BOARD_SIZE*(LOABoard.BOARD_SIZE-1)];
      }
    ...
    public void initialize(){
      ...
      //put in white
      for (int i=1;  i< LOABoard.BOARD_SIZE-1; i++) {
        currentHashKey ^= hashBoard[PLAYER_WHITE][LOABoard.BOARD_SIZE*i];
        currentHashKey ^= hashBoard[PLAYER_WHITE][LOABoard.BOARD_SIZE*i+(LOABoard.BOARD_SIZE-1)];
      }
    ...
    public void makeMove(int x1, int y1, int x2, int y2, boolean capture, int colorMoved) {
        ...
        //if a capture, also xor hashValue(square of piece captured)
         if (capture)
            currentHashKey = currentHashKey ^ hashBoard[colorMoved][LOABoard.BOARD_SIZE*y1+x1] ^
                             hashBoard[colorMoved][LOABoard.BOARD_SIZE*y2+x2] ^ hashBoard[opponentColorMoved][LOABoard.BOARD_SIZE*y2+x2];
        else
            currentHashKey = currentHashKey ^ hashBoard[colorMoved][LOABoard.BOARD_SIZE*y1+x1] ^
                             hashBoard[colorMoved][LOABoard.BOARD_SIZE*y2+x2];
    }
     ...
     */
    public void RecordHash(int depth, double value, int flag, LOAMove best, LOABoard brd) {
    ...
    public Move ProbeMove() {
      ...
      if (localHashElement.key != getHashKey())
    	return null;
      LOAMove m = new LOAMove(localHashElement.mX1, localHashElement.mY1, localHashElement.mX2, localHashElement.mY2, (int)localHashElement.value, localHashElement.p);
      return m;
    }
    ...
    private class HashElement {
        ...
        public HashElement(long key, int depth, int flag, double value, LOAMove best, LOABoard brd) {
                this.key = key;
                this.depth = depth;
                this.flag = flag;
                this.value = value;
